...
<template>
  <div class="flex flex-col mt-3 flex-1">
    <DetailHeader :title="$t('tools.coin.title')"></DetailHeader>
    ...
    <div class="p-4 rounded-2xl bg-white">
      ...
      <div class="flex justify-center">
        <el-button type="primary" class="w-48" size="large" @click="throwCoin" v-if="!genStatus">{{ $t('tools.coin.flip') }}</el-button>
        <el-button type="primary" class="w-48" size="large" disabled v-else>{{ $t('tools.coin.flipping') }}</el-button>
      </div>
    </div>

    <!-- desc -->
    <ToolDetail :title="$t('tools.coin.detail_title')">
      <el-text>
        {{ $t('tools.coin.detail_content') }}
      </el-text> 
    </ToolDetail>
